"""
XGBoost Model Training Script V2 - Using Real Historical Data.

This script:
1. Loads real historical K-line data from PostgreSQL
2. Generates asymmetric three-class labels (bullish/bearish, discard neutral)
3. Calculates technical indicator features
4. Trains XGBoost classifier with time-series split
5. Evaluates model performance
6. Saves model v2 and metadata

Key Improvements over V1:
- Real market data instead of synthetic data
- Asymmetric labeling strategy optimized for bull market
- Time-series split to prevent data leakage
- Comprehensive performance metrics

Usage:
    python services/decision_engine/scripts/train_xgboost_v2.py
"""
...
def train_model_v2(
    ...
    # ============================================
    logger.info("\nStep 1: Loading real historical K-line data...")
    ...
    logger.info(f"  - Stability measures: max_delta_step=1, gamma∈[0.1,0.2], lr≤0.1")
    ...
    if metrics['auc'] >= 0.60:
        logger.info("✅ AUC >= 0.60: EXCELLENT - Ready for A/B test (30% traffic)")
        gate_status = "PASS_EXCELLENT"
    elif metrics['auc'] >= 0.58:
        logger.info("⚠️  AUC >= 0.58: ACCEPTABLE - A/B test with 48h monitoring")
        gate_status = "PASS_ACCEPTABLE"
    else:
        logger.error("❌ AUC < 0.58: INSUFFICIENT - Do NOT deploy")
        gate_status = "FAIL"
    ...
    if pred_bullish_ratio < 0.10:
        logger.warning("  ⚠️  Model is EXTREMELY CONSERVATIVE (<10% bullish predictions)")
    elif pred_bullish_ratio < 0.20:
        logger.warning("  ⚠️  Model is CONSERVATIVE (<20% bullish predictions)")
    elif 0.20 <= pred_bullish_ratio <= 0.50:
        logger.info("  ✅ Prediction distribution is REASONABLE")
    else:
        logger.warning("  ⚠️  Model is AGGRESSIVE (>50% bullish predictions)")
    ...
    # Save model
    model_path = model_dir / "xgboost_signal_confidence_v2.pkl"
    ...
    metrics_with_metadata = {
        ...
        'training_info': {
            ...
            'model_version': 'v2.5-threshold-test',
...
if __name__ == "__main__":
    ...
    if auc >= 0.60:
        logger.info(f"✅ Model performance EXCELLENT (AUC={auc:.4f} >= 0.60). Success!")
        sys.exit(0)
    elif auc >= 0.58:
        logger.warning(f"⚠️  Model performance ACCEPTABLE (AUC={auc:.4f} >= 0.58). Recommend A/B testing with 30% traffic.")
        sys.exit(0)
    else:
        logger.error(f"❌ Model performance INSUFFICIENT (AUC={auc:.4f} < 0.58). Entering diagnostic mode.")
        sys.exit(1)
