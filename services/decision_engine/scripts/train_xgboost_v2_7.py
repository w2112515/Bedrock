"""
XGBoost Model Training Script V2.7 - Cross-Pair Features.

This script:
1. Loads real historical K-line data from PostgreSQL (1h + 4h + cross-pair)
2. Generates asymmetric three-class labels (bullish/bearish, discard neutral)
3. Calculates technical indicator features with cross-pair leading indicators
4. Trains XGBoost classifier with time-series split
5. Evaluates model performance
6. Saves model v2.7 and metadata

Key Improvements over V2.6:
- Cross-pair leading/lagging features (BTC/ETH leading indicators)
- Total 30 features (19 from v2.6 + 11 cross-pair)
- Tests hypothesis: BTC/ETH leading indicators improve prediction

Usage:
    python services/decision_engine/scripts/train_xgboost_v2_7.py --seed 42
"""
...
def train_model_v2(
    ...
    # ============================================
    # Step 1: Load Real Historical Data (Cross-Pair)
    # ============================================
    logger.info("\nStep 1: Loading real historical K-line data (cross-pair features)...")

    loader = RealDataLoader(db_url=settings.DATABASE_URL)
    samples = loader.load_klines_crosspair(
    ...
    for primary_window, secondary_window, reference_windows, symbol, timestamp, idx in samples:
        processed_count += 1

        if processed_count % 1000 == 0:
            logger.info(f"Processed {processed_count}/{len(samples)} samples...")

        # Calculate features (cross-pair)
        features = FeatureEngineer.calculate_features_crosspair(
            primary_window,
            secondary_window,
            reference_windows,
            symbol
    ...
    X_df = pd.DataFrame(X)
    feature_names = FeatureEngineer.get_feature_names_crosspair()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train XGBoost model v2.7 with cross-pair features')
